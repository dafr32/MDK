...
(function ($, Drupal, DrupalSettings){
    ...
    function init(){
        // Wyodrębnij datę ze ścieżki URL
        var path = window.location.pathname;        
        var pathParts = path.split('/');
        var myDateFromURL = pathParts[pathParts.length - 1];              
       
        var initialDate;

        // Spróbuj sparsować datę
        try {
            initialDate = myDateFromURL ? new Date(myDateFromURL) : new Date();
            // Sprawdź, czy data jest prawidłowa
            if (isNaN(initialDate.getTime())) {
                throw new Error("Invalid date");
            }
        } catch (error) {            
            // Ustaw domyślną datę (np. dzisiejszą datę) w przypadku błędu
